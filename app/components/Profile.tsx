      ...
      <AnimatePresence>
        {openProfile && (
          <motion.div
            ...
            exit={{ scale: 0.9, opacity: 0, transition: { duration: 0.18, ease: "easeIn" } }}
          ...
          >
            <p className="text-black dark:text-slate-100 font-semibold">{user.name}</p>
            <p className="text-sm text-slate-500 dark:text-slate-300">{user.email}</p>
            <div className="mt-2 border-t pt-2">
              <Link href="/private" className="block px-2 py-1 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded">My Profile</Link>
              <Link href="/" className="block px-2 py-1 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded">Dashboard</Link>
              <button className="w-full text-left px-2 py-1 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded">Logout</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
