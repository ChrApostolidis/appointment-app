...
export default function AppointmentCard({bookings}: {bookings: Bookings[]}) {
  return (
    <div className="mt-4 bg-foreground w-[90%] lg:max-w-xl lg:w-full p-3 rounded-xl">
      ...
      <div className="my-5 flex flex-col lg:flex-row gap-2 lg:gap-4 lg:justify-between">
        <div className="flex gap-1">
          ...
          <p className="text-background">{formatTime(bookings[0].startAt)} - {formatTime(bookings[0].endAt)}</p>
        </div>
        <div className="flex gap-1">
          <Calendar fontSize={14} className="text-background" />
          <p className="text-background">{bookings[0].startAt.toLocaleDateString()}</p>
        </div>
        <div className="flex gap-1">
